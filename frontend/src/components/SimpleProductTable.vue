              ...
              <tr :class="{ 'group-row': item.isGroup, 'task-row': !item.isGroup }">
                <td class="col-name">
                  ...
                  >
                    ...
                    <el-icon v-if="item.isGroup" class="group-icon"><Box /></el-icon>
...
function onTaskBarMouseDown(e, item) {
  ...
  const onMove = (e2) => {
    ...
    const barWidth = Math.max(((new Date(item.end_date) - new Date(item.start_date)) / (24 * 60 * 60 * 1000)) * dayWidth.value, 20)
...
const groupedByProduct = computed(() => {
  ...
  topLevelItems.forEach(product => {
    // 产品名称就是顶级项的 text
    const productName = product.text || `产品 ${product.id}`
    
    // 找出该产品下的所有子任务（工序）（只在当前传入的数据中查找，确保是过滤后的数据）
    const children = data.filter(d => d.parent === product.id)
    
    // 计算合并后的时间段（用于产品行显示多段条形图）
    ...
    productList.push({
      ...
      expanded: expandedItems[product.id] === true // 默认收起
    })
  })
...
// 计算产品的实际占用时间范围（根据子任务的时间）
const calculateProductTimeRange = (children) => {
...
const flattenedData = computed(() => {
  ...
  groupedByProduct.value.forEach(product => {
    // 添加产品行（第一级）
    const isExpanded = expandedItems[product.id] === true // 默认收起
    
    // 计算子任务的时间范围（仅用于显示开始时间列）
    const timeRange = calculateProductTimeRange(product.children)
    ...
    result.push({
      ...
      // 产品行使用多段时间范围
      timeRanges: product.timeRanges,
      // 用于在左侧表格"开始时间"列显示
      displayStartDate: timeRange.start_date,
      level: 0,
      isGroup: true,
      expanded: isExpanded,
      durationDisplay: calculateTotalDuration(product.children),
      text: product.productName
    })
    
    // 如果展开，添加子任务（第二级 - 工序）
    if (isExpanded && product.children.length > 0) {
      product.children.forEach(task => {
        result.push({
          ...task,
...
// 计算总时长：优先用作业时间 work_hours（与排程前一致）
